...
// This date is used throughout to determine today's date.
    ...
    var IONBaseYear        = IONDateNow.getFullYear()-75;

    // How many years do want to be valid and to show in the drop-down list?

    var IONDropDownYears   = 167;
    ...
    var IONToday               = 'Today:',
        ...
        IONArrMonthNames =['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep',
									'Oct','Nov','Dec'],                          
    ...
    var IONBackground           = '#959595';    // Calendar background
    var IONHeadText             = 'White';    // Colour of week headings
    ...
    // Today string
    var IONTodayText            = 'White',
        IONTodayHighlight       = 'Blue';
    ...
    document.writeln(   '.ION       {padding:0px;vertical-align:middle;}');
    ...
    document.writeln(   'table.ION  {padding:0px;visibility:hidden;'            +
                                    ...
                                    ';border:ridge 2px DarkGray;font-size:7pt;'         +
                                    'font-family:Verdana,Arial,Helvetica,Sans-Serif;'   +
                                    'font-weight:bold;}');
    document.writeln(   'td.IONHead     {padding:0px 0px;text-align:center;}');
    document.writeln(   'select.IONHead {margin:0px 0px;}');
    document.writeln(   'input.IONHead  {height:21px;width:20px;'               +
                                        'vertical-align:middle;'                +
                                        'text-align:center;margin:0px 0px;'     +
                                        'font-size:7pt;borderwidth:0;font-family:fixedSys;'  +
                                        'font-weight:normal;}');
    document.writeln(   'tr.IONWeek     {text-align:center;font-weight:bold;'   +
                                        'color:' + IONHeadText + ';}');
    document.writeln(   'td.IONWeek     {padding:0px;}');
    document.writeln(   'table.IONCells {text-align:right;font-size:7pt;'       +
                                        'width:96%;font-family:'                +
                                        'Arial,Helvetica,Sans-Serif;}');
    document.writeln(   'td.IONCells {cursor:hand;padding:0px;vertical-align:middle;'       +
                                     'width:16px;height:16px;font-weight:bold;' +
                                     'color:' + IONCellText                     +
                                     ';background-color:' + IONCellBackground   +
                                     '}');
    document.writeln(   'td.IONFoot  {cursor:hand;font-size:7pt;padding:0px;text-align:Left;'           +
...
*/
         ...
         document.write(IONDisplayDate.getFullYear()+ "-" +
                          IONArrMonthNames[IONDisplayDate.getMonth()]               + "-" +
    ...
    function IONSetOutput(IONOutputDate)
        {// Numeric months are held internally as 0 to 11 in this script so
         ...
         IONTargetEle.value =   ((IONOutputDate.getDate()<10)?'0':'')  +
			...
			*/
         // for Month, Day and Year output use...
            //change to YYYY/MM/DD for Systeam
        
         IONTargetEle.value = IONOutputDate.getFullYear()   + '-' +
                                     IONArrMonthNames[IONOutputDate.getMonth()]    + '-' +
                                ((IONOutputDate.getDate()<10)?'0':'')  +
                                IONOutputDate.getDate()     ;
//alert(IONTargetEle.value);
         IONHide();
        }
        function IONSetOutputClear()
        {
         IONTargetEle.value ='';
         IONHide();
        }
...
//------------------------------------------------------------------------------
   ...
   //Select the current year
	     ...
	     document.getElementById('IONYears').options.selectedIndex =75
            //Math.floor(IONMonthSum/12);
            
            var IONShowDate  = new Date(Date.parse(new Date().toDateString()));
           
         document.getElementById('IONMonths').options.selectedIndex=IONShowDate.getMonth();     
            //(IONMonthSum%12);
         ...
         do {IONEle=IONEle.parentNode;
             if (IONEle.tagName!='FORM'  &&
                ...
                {offsetTop +=parseInt(IONEle.offsetTop,10);
                 ...
                 if(IONEle.tagName=='DIV')
                {
					offsetTop-=IONEle.scrollTop;
                }
   ...
   function IONFootOutputClear()
   {
    ...
    function IONFootOverClear()
        {document.getElementById('IONFootClear').style.color=IONTodayHighlight;
         //document.getElementById('IONFootClear').style.fontWeight='bold';
        }

    function IONFootOutClear()
        {document.getElementById('IONFootClear').style.color=IONTodayText;
         document.getElementById('IONFootClear').style.fontWeight='normal';
        }
...
//alert(IONBias);
		 ...
		 if ((12*parseInt((IONShowDate.getFullYear()-IONBaseYear),10)) +
            ...
            var IONFootClear = document.getElementById('IONFootClear'); 
             ...
             if (IONDisabledDates.length==0)
                {if (IONActiveToday)
                    {IONFoot.onclick=IONFootOutput;
                     ...
                     IONFootClear.onclick=IONFootOutputClear;
                     IONFootClear.onmouseover=IONFootOverClear;
                     IONFootClear.onmouseout =IONFootOutClear;                     
                    }
                 else
                    {if (document.addEventListener)
                     ...
                     IONFootClear.onmouseover=null;
                     IONFootClear.onmouseout =null;   
                    }
                }
             else
    ...
    if ((new Date(IONBaseYear + IONDropDownYears, 11, 32)) > IONDateNow &&
        ...
        {document.write(
                  "<tfoot class='IONFoot'>" +
                    "<tr class='IONFoot'><td colspan='7'><table border=0 cellpadding=0 cellspacing=0><tr>" +
                      "<td align=left class='IONFoot' id='IONFoot'>" + IONToday + " ");
                         IONDisplayFormat(IONDateNow);
        
                         document.write(
                        "</td><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td class='IONFoot' id='IONFootClear'>Clear</td>" +
