#Region "Purpose"
'************************************************************************************************************
' Page :                 :- Set Communication rule on particular call 
' Purpose                :- Create textbox on runtime based on datagrid columns
' Tables                 :- T010011, EventMaster, EventUserMaster, UDC, T040081, T060011, T070031, T210011,                              setup_rules, priorityobject  
' Date					   Author						Modification Date				Description
' 05/03/06			  	   jagtar                             				            Created
' Tables used
' Notes: 
' Code:
'************************************************************************************************************
...
'Session("UNameComn") replace with ViewState("Commu_UserName")
'Session("RNameComn")replace with ViewState("Commu_RoleName")
#End Region
...
Partial Class CommunicationSetup_frm_CommunicationSetup
#Region "Varibles"
    Inherits System.Web.UI.Page
    Private mtxtUDCTypeQuery As TextBox()
    'Protected WithEvents BtnGrdSearch As Button
...
#Region "Page_Load"
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If (Not Page.IsPostBack) Then
            ...
            End If
            PopulateDropDownLists("select Event_ID_PK, Name from EventMaster where Enabled = 'Y' and Event_ID_PK<>18  and Commn_Escl not in('E')", ddEventName, "N")
            PopulateDropDownLists("select Event_User_ID_PK, Event_User_Name from EventUserMaster", ddEventUser, "N")
        ...
        Try
            ...
            If Not IsPostBack Then
                ...
                If (Session("PropCompanyType") = "SCM") Then
                    str = "Select sr.Table_ID as RNo,sr.Mail_on_off as M,sr.SMS_on_off as S,convert(varchar,sr.Start_Date,101) as StartDate,convert(varchar, sr.Stop_Date,101) as StopDate,em.Name as Event, Event_User_Name as DefaultUser,em1.name as OnEvent,ab.CI_VC36_Name as Comp,un.UM_VC50_UserID as UserName,ROM_VC50_Role_Name as Role,sr.Priority,sr.Call_Type as CType, call.SU_VC50_Status_Name as CStatus, sr.Task_type as TType, task.SU_VC50_Status_Name as TStatus, pR.PR_VC20_Name as SubCat, sr.Rule_Status as Stat from eventmaster em, eventmaster em1, Setup_rules sr,T040081 call, T040081 task, t010011 ab, t070031 rn, t060011 un, Eventusermaster eum,T210011 as PR where sr.Event_ID_FK *= em.Event_ID_PK and sr.Event_User_ID_FK *= eum.Event_User_ID_PK and sr.Role_id *= rn.ROM_IN4_Role_ID_PK and sr.Company_id *= ab.CI_NU8_Address_Number and sr.user_id *= un.UM_IN4_Address_No_FK and sr.Event_Fired_ID_FK *= em1.Event_ID_PK and sr.Call_Status *= call.SU_VC50_Status_Name and sr.Task_Status *= task.SU_VC50_Status_Name and sr.Project_ID*=PR.PR_NU9_Project_ID_Pk and sr.company_id*=PR_NU9_Comp_ID_Fk and sr.Event_ID_FK <> 18   and record_type='C'"
                Else
                    str = "Select sr.Table_ID as RNo,sr.Mail_on_off as M,sr.SMS_on_off as S,convert(varchar,sr.Start_Date,101) as StartDate,convert(varchar, sr.Stop_Date,101) as StopDate,em.Name as Event, Event_User_Name as DefaultUser,em1.name as OnEvent,ab.CI_VC36_Name as Comp,un.UM_VC50_UserID as UserName,ROM_VC50_Role_Name as Role,sr.Priority,sr.Call_Type as CType, call.SU_VC50_Status_Name as CStatus, sr.Task_type as TType, task.SU_VC50_Status_Name as TStatus, PR.PR_VC20_Name as SubCat, sr.Rule_Status as Stat from eventmaster em, eventmaster em1, Setup_rules sr,T040081 call, T040081 task, t010011 ab, t070031 rn, t060011 un, Eventusermaster eum,T210011 as PR where sr.Event_ID_FK *= em.Event_ID_PK and sr.Project_ID*=PR.PR_NU9_Project_ID_Pk and sr.company_id*=PR_NU9_Comp_ID_Fk and sr.Event_User_ID_FK *= eum.Event_User_ID_PK and sr.Role_id *= rn.ROM_IN4_Role_ID_PK and sr.Company_id *= ab.CI_NU8_Address_Number and sr.user_id *= un.UM_IN4_Address_No_FK and sr.Event_Fired_ID_FK *= em1.Event_ID_PK and sr.Call_Status *= call.SU_VC50_Status_Name and sr.Task_Status *= task.SU_VC50_Status_Name and sr.Event_ID_FK <> 18   and record_type='C' and (sr.Company_id=0 or sr.Company_id=" & Session("PropCompanyID") & ")"

                End If

                str &= " and Company_id  in (" & GetCompanySubQuery() & ")  "

                If SQL.Search("eventmaster", "", "", str, dsFromView, "", "") Then
        ...
        Catch ex As Exception
            CreateLog("ComSetup", "Page_Load-451", LogType.Application, LogSubType.Exception, ex.TargetSite.Attributes, ex.Message, HttpContext.Current.Session("PropUserID"), HttpContext.Current.Session("PropUserName"), "NA")
        End Try
    End Sub
#End Region
...
#Region "isExisted"
    Private Function isExisted() As Boolean
        ...
        Try
            ...
            QueryString = QueryString & " and record_type='C' and Rule_Status = 1"
            ...
            End If
        Catch ex As Exception
            CreateLog("ComSetup", "isExisted-451", LogType.Application, LogSubType.Exception, ex.TargetSite.Attributes, ex.Message, HttpContext.Current.Session("PropUserID"), HttpContext.Current.Session("PropUserName"), "NA")
            Return False
        Finally
            objCon.Close()
        End Try
...
#Region "GetPriorityParams"
    Private Function GetPriorityParams() As Boolean
        ...
        Catch ex As Exception
            ...
            CreateLog("ComSetup", "ShowValues-413", LogType.Application, LogSubType.Exception, ex.TargetSite.Attributes, ex.Message, HttpContext.Current.Session("PropUserID"), HttpContext.Current.Session("PropUserName"), "NA", False)
...
#Region "GetEventStatus"
    Private Function GetEventStatus() As Boolean
        ...
        If IsNothing(ViewState("EventStatus")) Then
            ...
            Dim objCon As SqlConnection = New SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings("ConnectionString").ToString)
            Try
                objCon.Open()
            ...
            Catch ex As Exception
                ...
                CreateLog("ComSetup", "ShowValues-451", LogType.Application, LogSubType.Exception, ex.TargetSite.Attributes, ex.Message, HttpContext.Current.Session("PropUserID"), HttpContext.Current.Session("PropUserName"), "NA", False)
        ...
        Try
            ...
            Else
                ...
                If ddData.ID = "ddUserName" Then
                    ...
                    ViewState.Add("Commu_UserName", dtDDData)
                ElseIf ddData.ID = "ddRoleName" Then
                    If dtDDData.Rows.Count > 0 Then
                        dtDDData.Rows(0).Delete()
                    End If
                    ''Session.Add("RNameComn", dtDDData)
                    ViewState.Add("Commu_RoleName", dtDDData)
                End If
            End If

            'ddData.DataBind()
        Catch ex As Exception
            CreateLog("frmcommsetup", "PopulateDropDownLists-486", LogType.Application, LogSubType.Exception, ex.TargetSite.Attributes, ex.Message, HttpContext.Current.Session("PropUserID"), HttpContext.Current.Session("PropUserName"), "NA")
        Finally
            sqlCon.Close()
        End Try
    ...
    Private Function SaveSetupRule(ByVal ColumnName As ArrayList, ByVal RowData As ArrayList, ByVal TableName As String) As ReturnValue
        ...
        Try
            ...
            If dtEndDate.Text.Trim <> "" Then
                ...
                Else
                    'cpnlError.Visible = True
                    lstError.Items.Add("Comm. Setup Stop Date cannot be less than Started date...")
            ...
            Else
                ...
                End If
            End If
            If SQL.Save(TableName, "frm_CommSetup", "SaveSetupRule", ColumnName, RowData) = False Then
                lstError.Items.Clear()
...
#Region "ClearControls"
    Private Sub ClearControls()
        Try
            ...
            dtEndDate.Text = ""
        Catch ex As Exception
            CreateLog("frm_commsetup", "ClearControls-878", LogType.Application, LogSubType.Exception, ex.TargetSite.Attributes, ex.Message, Session("PropUserID"), Session("PropUserName"), "NA")
        End Try
    End Sub
#End Region
...
#Region "fill View"
    ...
    Private Sub FillView()
        ...
        If SQL.Search("Harpreet", "frm_commsetup", "FillView", str, dsFromView, "", "") Then
            Try
                mdvtable.Table = dsFromView.Tables(0)
                If dgCommRules.AutoGenerateColumns = False Then
                    dgCommRules.AutoGenerateColumns = True
        ...
        Try
            ...
            For intii = 0 To intCol - 1
                ...
                If Not IsPostBack Then
                    ...
                    'anel1.Controls.Add(Page.ParseControl("<asp:TextBox id=" & arCol.Item(intii) & " runat=""server"" Width=""0"" CssClass=SearchTxtBox></asp:TextBox>"))
                    'Else
...
#Region "Format datagrid columns size according to database"
    ...
    Private Sub FormatGrid()
        ...
        Catch ex As Exception
            CreateLog("frm_CommSetup", "FormatGrid-643", LogType.Application, LogSubType.Exception, ex.TargetSite.Attributes, ex.Message, HttpContext.Current.Session("PropUserID"), HttpContext.Current.Session("PropUserName"))
        End Try
    End Sub
...
#Region "ddCompany_SelectedIndexChanged"
    Private Sub ddCompany_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ddCompany.SelectedIndexChanged
        ...
        dtDDData = ViewState("Commu_UserName")
        ddUserName.Items.Clear()
        'use want all user on selected company
        strFilter = ""
        ...
        dtDDData = ViewState("Commu_RoleName")
        ddRoleName.Items.Clear()
        'PopulateDropDownLists("select ROM_IN4_Role_ID_PK ,ROM_VC50_Role_Name,ROM_IN4_Company_ID_FK
...
#Region "PopulateCommunicationSetupGrid"
    Private Sub PopulateCommunicationSetupGrid()
        ...
        If SQL.Search("Setup_Rules", "frm_commsetup", "PopulateCommunicationSetupGrid", "Select * from Setup_Rules", ds, "", "") = True Then
...
#Region "Serach Grid Button Click"
    ...
    Private Sub BtnGrdSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnGrdSearch.Click
        ...
        Try
            ...
            GetColumns()
        Catch ex As Exception
            CreateLog("frm_commsetup", "Click-693", LogType.Application, LogSubType.Exception, ex.TargetSite.Attributes, ex.Message, HttpContext.Current.Session("PropUserID"), HttpContext.Current.Session("PropUserName"), "BtnGrdSearch")
        End Try
    End Sub
        ...
        Try
            ...
            For Each dcCol In mdvtable.Table.Columns
                If e.Item.ItemType = ListItemType.AlternatingItem Or e.Item.ItemType = ListItemType.Item Then
                    If e.Item.Cells(17).Text = 0 Then
            ...
            If e.Item.ItemType = ListItemType.AlternatingItem Or e.Item.ItemType = ListItemType.Item Then
                ...
                strCom = e.Item.Cells(8).Text.Trim
                If (Session("PropCompanyType") = "SCM") Then
            ...
            rowvalue += 1
        Catch ex As Exception
            CreateLog("frm_commsetup", "ItemDataBound-807", LogType.Application, LogSubType.Exception, ex.TargetSite.Attributes, ex.Message, HttpContext.Current.Session("PropUserID"), HttpContext.Current.Session("PropUserName"), "dgCommRules")
        End Try
    End Sub
